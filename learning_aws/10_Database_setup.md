...
## 1. 概要
本演習では、Amazon RDS (MySQL 8.0) を構築するための各種グループ設定およびインスタンスの作成を行います。
高可用性を意識し、マルチAZ構成を見据えた「DBサブネットグループ」の作成を含みます。
...
- **サブネットグループの重要性:** -  RDSを起動するには、異なるAZに属する最低2つのサブネットが必要です。これにより、AWS側で障害が発生した際のフェイルオーバー（マルチAZ）が可能になります。
